  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? "bg-white/70 backdrop-blur-2xl border-b border-white/20 shadow-sm"
          : "bg-transparent border-b border-transparent"
      }`}
    >
      <div className="container mx-auto flex h-16 items-center justify-between px-6">
        <Link to="/" className="flex items-center gap-2.5">
          <div className="h-8 w-8 rounded-[14px] bg-gradient-to-br from-mint-200 to-mint-400 flex items-center justify-center">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors px-2 py-1.5 rounded-[14px] hover:bg-muted"
          >
          ...
          <Link to="/pricing" className="rounded-[14px] btn-glow px-5 py-2 text-sm font-medium text-foreground transition-all hover:opacity-90 active:scale-[0.98]">
            {t.nav.getStarted}
          </Link>
        </div>
      </div>
