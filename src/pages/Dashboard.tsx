              ...
              {insights.map((insight, i) => (
                <div key={i} className={`${i === 1 ? "glass-bento" : "glass-card"} p-5 flex items-center justify-between gap-4`}>
                  <div className="flex items-start gap-3 flex-1">
                    <div className={`rounded-[5px] p-2 shrink-0 ${insight.urgency === "Critical" ? "bg-destructive/15" : insight.urgency === "High" ? "bg-primary/15" : "bg-white/[0.06]"}`}>
              ...
              {executives.map((exec, i) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    <img src={exec.avatar} alt={exec.name} className="h-11 w-11 rounded-[5px] object-cover ring-2 ring-white/[0.08]" />
                    ...
                    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-[5px] ${statusColor(exec.status)}`}>
                  ...
                  <div className="flex gap-2">
                     <Link to={`/expert/${exec.id}`} className="flex-1 text-center text-xs py-2 rounded-[5px] bg-white/[0.05] hover:bg-white/[0.08] text-foreground transition-colors">
                      {t.dashboard.open}
                    </Link>
                    <button className="flex-1 text-xs py-2 rounded-[5px] bg-primary/15 hover:bg-primary/25 text-primary transition-colors">
                      {t.dashboard.assign}
                    </button>
                  </div>
                </motion.div>
            ...
            <div className="glass-bento p-6">
              <div className="grid grid-cols-5 gap-3">
                {weeklyRhythm.map((day, i) => (
                  <div key={i} className="text-center p-4 rounded-[5px] bg-white/[0.03] hover:bg-white/[0.06] transition-colors">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {impact.map((item, i) => (
                <div key={item.label} className={`${i === 0 || i === 3 ? "glass-bento" : "glass-card"} p-5 text-center`}>
                  <div className="mx-auto w-10 h-10 rounded-[5px] bg-primary/10 flex items-center justify-center mb-3">
